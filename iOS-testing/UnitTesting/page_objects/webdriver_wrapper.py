...
class webdriver_wrapper():
    ...
    def __init__(self, browser, rand_username_int=None):
        global _drivers, mode
        if rand_username_int is None:
            self.rand_username_int = str( random.randint(0,1000000) )
        else:
            self.rand_username_int = rand_username_int
        ...
        #below, use for window zoomout
        self._driver.get(authURL)
        _zola = self._driver.find_element_by_xpath('/html/body/div[2]/header/div/div/a')
        for x in range(0,5):
            _zola.send_keys(Keys.COMMAND, Keys.SUBTRACT)
    ...
    def check_url(self, _url):
        ...
        if _url != _actual_url:
            raise AssertionError('Current URL is "%s" but expected URL is "%s"' % (_url, _actual_url))
