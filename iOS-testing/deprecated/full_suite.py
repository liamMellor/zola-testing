...
class test_app_full_suite(unittest.TestCase):
    ...
    def light_purchase(self, webd_wrap):
    ...
    def test_light_purchase(self): #running x as a unit test
        for browser in webdriver_wrapper._browsers:
            self.light_purchase(webdriver_wrapper(browser))
    
    
    def test_ui_comp(self):
        for version in appium_wrapper.versions:
            appium = appium_wrapper(version)
            login_screen_inst = login_screen(appium)
