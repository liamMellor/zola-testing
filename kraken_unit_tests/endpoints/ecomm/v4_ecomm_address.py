...
class v4address():
    ...
    def __init__(self, auth_member_id, auth_token, action=None, objType=None, id=None, country_iso=None, order_id=None, name=None, address1=None, address2=None, city=None, state=None, zip=None, country_alpha2=None, label=None, card_name=None, card_type=None, card_exp_month=None, card_exp_year=None, card_cvv=None, card_number=None):
...
class addressTest():
    ...
    def address(self):
		...
		#######################################   Action == LIST   #######################################
        for objType in listTypes:
            if objType == 'zip':
                testAddress = v4address(self.auth_member_id, self.auth_token, 'list', objType, 11206, '840', None, "Liam Mellor", "36 Meserole St", None, "Brooklyn", "NY", "11206", None, None, "Liam Mellor", "Visa", None, None, None, None)
                testAddress.address(self.api_url)
            else:
                testAddress = v4address(self.auth_member_id, self.auth_token, 'list', objType, None, '840', None, "Liam Mellor", "36 Meserole St", None, "Brooklyn", "NY", "11206", None, None, "Liam Mellor", "Visa", None, None, None, None)
                testAddress.address(self.api_url)
        ...
        for objType in otherTypes:
            if objType == 'update' or objType == 'set-default':
                testAddress = v4address(self.auth_member_id, self.auth_token, 'shipping', objType, id, '840', 2, "Liam Mellor", "36 Meserole St", None, "Brooklyn", "NY", "11206", None, None, "Liam Mellor", "Visa", 06, 2017, 333, 5555555555554444)
                testAddress.address(self.api_url)
            else:
                testAddress = v4address(self.auth_member_id, self.auth_token, 'shipping', objType, None, '840', 2, "Liam Mellor", "36 Meserole St", None, "Brooklyn", "NY", "11206", None, None, "Liam Mellor", "Visa", 06, 2017, 333, 5555555555554444)
                testAddress.address(self.api_url)
        ...
        # Check to make sure card was added correctly. There was a LIDL issue at one time that made this call fail. Not Zola's fault. 
		if type(value) is str:
			print "CHECK LINE 135. ACTION = BILLING TYPE = ADD DID NOT RETURN DICTIONARY OF VALUES (returned string). IT FAILED GRACEFULLY. COULD BE LIDL ISSUE."
		else:
			print "all is well with LIDL."
			objects     = value[1]['data']
			mostRecent  = objects[-1]
	        id          = mostRecent['id']
	        ...
	        for objType in otherTypes:
	            if objType == 'update' or objType == 'set-default':
	                testAddress = v4address(self.auth_member_id, self.auth_token, 'billing', objType, id, '840', 2, "Liam Mellor", "36 Meserole St", None, "Brooklyn", "NY", "11206", None, None, "Liam Mellor", "Visa", 06, 2017, 333, 5555555555554444)
	                testAddress.address(self.api_url)
	            else:
	                testAddress = v4address(self.auth_member_id, self.auth_token, 'billing', objType, None, '840', 2, "Liam Mellor", "36 Meserole St", None, "Brooklyn", "NY", "11206", None, None, "Liam Mellor", "Visa", 06, 2017, 333, 5555555555554444)
	                testAddress.address(self.api_url)
        ...
        #######################################   Action == RETURN CHECK   #######################################
		# action = return-check
        testAddress = v4address(self.auth_member_id, self.auth_token, 'return-check', objType, None, '840', None, "Liam Mellor", "36 Meserole St", None, "Brooklyn", "NY", "11206", None, None, "Liam Mellor", "Visa", None, None, None, None)
        testAddress.address(self.api_url)
